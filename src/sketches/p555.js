...
function p555({ name, width, height, closed }) {
    ...
    p.draw = function() {
      if (!closed.current) {
        ...
        for (let i = 0; i < 200; i++) {
          ...
          let x =
            (p.width / 8 + i * p.map(p.sin(speed), -p.PI, p.PI, 0, i / 10)) * p.sin(speed + i / 100) +
            p.noise(speed) * 100
          let y =
            (p.height / 8 - i * p.map(p.cos(speed), -p.PI, p.PI, 0, i / 10)) * p.cos(speed + i / 100) +
            p.noise(speed) * 100
